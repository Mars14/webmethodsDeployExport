...
module.exports = function (context) {
    ...
    function exportPageMethod(promise, methodName) {
        ...
        promise[methodName] = function () {
            ...
            return exportPageMethods(p.then(pageMethods[methodName].apply(originalP, arguments), function () {
...
function PageMethodsConstructor(context) {
    ...
    function waitForPageMessageJSON(eventId, resolve, dataName) {
        stateManager.waitForPageMessageJSON(context, eventId, dataName).then(resolve);
    }
    ...
    this.onGlobalMutation = function (func, allFrames) {
        return newPromiseWrapper(function (resolve) {
            logger.info("page.onGlobalMutation(function," + allFrames + ")");
    ...
    this.extract = function (pageModel, dataName) {
        return newPromiseWrapper(function (resolve) {
            var eventId = newEventId();
            waitForPageMessageJSON(eventId, resolve, dataName);
            ...
            context.page.evaluate(function (pageModel, eventId) {
                if (window.powwow) {
                    ...
                    powwow.extract(desc, powwow.getCurrentWindow().document).then(function (response) {
                        powwow.sendPageMessage(eventId, powwow.stringify(response));
                    });
                } else {
                    powwow.sendPageMessage(eventId, "{}");
                }
    ...
    this.extractAndTest = function (pageModel, funcTest) {
        return newPromiseWrapper(function (resolve) {
            var eventId = newEventId();
            waitForPageMessageWithoutDataMap(eventId, function(extractedDataJSON) {
                var extractedData = JSON.parse(extractedDataJSON);
                funcTest.call(this, extractedData);
                resolve();
            });
            logger.info("page.extractAndTest('" + pageModel + "');");
            context.page.evaluate(function (pageModel, eventId) {
                if (window.powwow) {
                    var desc = powwow.getDescriptor(powwow.descriptors, pageModel);
                    powwow.extract(desc, powwow.getCurrentWindow().document).then(function (response) {
    ...
    // Extract data from the page using the page model.
    this.extractStateDescriptor = function (pageModel, dataName) {
        return newPromiseWrapper(function (resolve) {
            var eventId = newEventId();
            waitForPageMessageJSON(eventId, resolve, dataName);
            logger.info("page.extractStateDescriptor('" + pageModel + "');");
            context.page.evaluate(function (pageModel, eventId) {
                if (window.powwow) {
                    var desc = powwow.getStateDescriptor(pageModel);
                    powwow.extract(desc, powwow.getCurrentWindow().document).then(function (response) {
                        powwow.sendPageMessage(eventId, powwow.stringify(response));
                    });
                } else {
            ...
            context.page.evaluate(function (eventId, testAll) {
                powwow.checkStateUsingDescriptor(testAll).then(function (data) {
                    if (testAll) {
                        ...
                        //console.log("Found state variant:", powwow.stringify(data));
                        powwow.sendPageMessage(eventId, data);
                    }
                });
    ...
    this.lockState = function () {
        return newPromiseWrapper(function (resolve) {
            logger.info("page.lockState();");
            stateDetector.lockState();
            resolve(context);
        });
    };
    ...
    this.unlockState = function () {
        return newPromiseWrapper(function (resolve) {
            logger.info("page.unlockState();");
            stateDetector.unlockState();
            resolve(context);
        });
    };

    this.checkState = function (force) {
        var page = this;
        return newPromiseWrapper(function (resolve) {
            logger.info("page.checkState(" + force + ");");
            stateDetector.checkState(page, null, force);
            resolve(context);
        });
    };
    ...
    this.onStateChange = function (func) {
        var page = this;
        return newPromiseWrapper(function (resolve) {
            logger.info("page.onStateChange();");
            stateDetector.events.on('newState', function (state, data) {
                func.call(this, state, data, resolve);
            });
        });
    };

    // Update data using a page model.  If no data is specified, the context data is used.
    this.update = function (pageModel, data, noFlatten) {
        return newPromiseWrapper(function (resolve) {
            var _pageModel = pageModel;
            var _data = data ? data : context.dataMap;
            var eventId = newEventId();
            waitForPageMessage(eventId, resolve);
            var logMsg = (logger.level() == logger.DEBUG) ? (", " + JSON.stringify(_data)) : "";
            ...
            context.page.evaluate(function (data, eventId, pageModel, noFlatten) {
                ...
                powwow.update(desc, data, powwow.getCurrentWindow().document, noFlatten).then(function () {
                    //powwow.log("powwow.update('" + pageModel + "',", powwow.stringify(data) + ")");
                    powwow.sendPageMessage(eventId);
                });
    ...
    this.reload = function () {
        return newPromiseWrapper(function (resolve) {
            ...
            context.page.reload(resolve);
        });
    };

    this.clearCookies = function () {
        return newPromiseWrapper(function (resolve) {
            logger.info("page.clearCookies();");
            context.page.clearCookies(resolve);
        });
    };

    this.resetStateDetection = function() {
        return newPromiseWrapper(function (resolve) {
            logger.info("stateDetector.resetStateDetection();");
            stateDetector.resetStateDetection();
            resolve(context);
        });
    }
    ...
    this.resizeWindow = function(width, height, dontResizeFrames) {
        return newPromiseWrapper(function (resolve) {
            context.page.evaluate(function(width, height, dontResizeFrames) {
                if(dontResizeFrames) {
                    window.innerHeight = height;
                    window.innerWidth = width;
                    if (window.contentDocument) {
                        window.contentDocument.body.clientWidth = width;
                        window.contentDocument.body.clientHeight = height;
                    }
                } else {
                    var arrFrames = powwow.getAllFrameWindows(window);
                    for (var i = 0; i < arrFrames.length; i++) {
                        ...
                        w.innerWidth = width;
                        if (w.contentDocument) {
                            w.contentDocument.body.clientWidth = width;
                            w.contentDocument.body.clientHeight = height;
                        }
                    }
                }
            }, width, height, dontResizeFrames, resolve);
        });
    };
    ...
    // Assign an existing object to the data object.
    this.assignData = function (dataToAssign) {
        return newPromiseWrapper(function (resolve) {
            logger.info("page.assignData();");
            _.assign(context.dataMap, dataToAssign);
            resolve(context);
        });
    };

    // Re-use the data last sent to the client.
    this.reuseLastData = function () {
        return newPromiseWrapper(function (resolve) {
            logger.info("page.reuseLastData();");
            context.dataMap = stateManager.getCurrentScreen().params;
            resolve(context);
        });
    };
    ...
    this.getScreen = function () {
        return stateManager.getCurrentScreen().name;
    };
    ...
    this.checkLoadedPage = function () {
        return newPromiseWrapper(function (resolve) {
            logger.info("page.checkLoadedPage();");
    ...
    // Get an image using a selector as a base64 data URL.
    this.getImageDataURL = function (imgSelector, dataItem) {
        return newPromiseWrapper(function (resolve) {
            ...
            }, imgSelector, function (err, rect) {
                context.page.set('clipRect', rect, function () {
                    context.page.renderBase64('PNG', function (err, base64EncodedImage) {
                        context.page.set('clipRect', {}, function () {
                            context.dataMap[dataItem] = 'data:image/png;base64,' + base64EncodedImage;
                            resolve(context);
                        });
                    });
                });
    ...
    // Get image of the page as base64.
    this.renderBase64 = function (type) {
        return newPromiseWrapper(function (resolve) {
            logger.info("page.renderBase64('" + type + "');");
            context.page.renderBase64(type, function (err, base64) {
                resolve(base64);
            });
        });
    };
    ...
    this.open = function (url) {
        return newPromiseWrapper(function (resolve) {
            logger.info("page.open('" + url + "');");
            pageService.createPhantomPage(phantomBrowser.getBrowser(), function (page) {
                stateManager.connectToPage(page);
    ...
    this.showLoading = function (message) {
        return newPromiseWrapper(function (resolve) {
            logger.info("page.showLoading(" + message + ");");
    ...
    this.externalApi = function(apiName) {
        return newPromiseWrapper(function (resolve) {
            var externalApi = require.main.require('./externalApi');
            if(externalApi && externalApi[apiName] && typeof(externalApi[apiName]) == 'function') {
                logger.info("page.externalApi('" + apiName + "');");
                try {
                    externalApi[apiName].call(externalApi[apiName], context.dataMap, resolve);
                } catch(e) {
                    logger.error("Error in external API call");
                    logger.error(e);
                    resolve(context);
                }
            } else {
                logger.error("Calling External API that does not exist.  Please make sure there is a externalApi/" + apiName + ".js file and that it is exported in externalApi/index.js");
                resolve(context);
            }
        });
    };

    this.uploadFile = function(selector, filePath) {
       return newPromiseWrapper(function (resolve) {
            context.page.uploadFile(selector, filePath, resolve);
       });
    };
