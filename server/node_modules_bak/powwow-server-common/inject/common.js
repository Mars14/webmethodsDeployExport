...
(function (powwow) {
        ...
        if (item.type == "descriptor") {
            if (item.descriptor) {
                ...
                if (subDescriptor) {
                    ...
                    for (var iProp; i < props.length; iProp++) {
                        prop = props[iProp];
    ...
    function runQuerySelector(node, field, operations) {
        ...
        if (jsType == "string") {
            ...
            if(isArray(foundNode)) {
                if(foundNode.length > 0) {
                    console.error("Query selector has a function that returns an array.  Returning the first item");
    ...
    function recursiveFlattenData(dataItem, itemId, newDataRoot) {
        ...
        } else if (isArray(dataItem) && dataItem.length > 0 && isObject(dataItem[0])) {
    ...
    powwow.waitOnceForSpecificDOMMutation = function (rootNode, funcMutationsTest, timeoutInMs) {
        ...
        return new powwow.Promise(function (resolve, reject) {
            var observer;
    ...
    powwow.getStateDescriptor = function (stateDescriptorId) {
        if (powwow.stateDescriptors) {
            for (var i = 0; i < powwow.stateDescriptors.length; i++) {
                if(powwow.stateDescriptors[i].id == stateDescriptorId) {
                    return powwow.stateDescriptors[i];
                }
            }
        } else {
            // This must be the Chrome Extension since otherwise we would have state descriptors defined.
            return powwow.descriptors[stateDescriptorId];
        }
    };
    ...
    powwow.extract = function (pageModel, scopedoc) {
        return new powwow.Promise(function (resolve) {
            ...
            try {
                ...
                powwow.log('Can not process the selector...' + pageModel.id);
                console.error(e);
            }
    ...
    powwow.setData = powwow.update = function (pageModel, data, scopedoc) {
        return new powwow.Promise(function (resolve) {
            if (!scopedoc) {
                scopedoc = document;
            }
    ...
    powwow.checkStateUsingDescriptor = function (testAll) {
        ...
        return new powwow.Promise(function (resolve) {
            ...
            if (powwow.stateDescriptors) {
                for (var i = 0; i < powwow.stateDescriptors.length; i++) {
                    models.push(powwow.stateDescriptors[i]);
                }
            } else {
                // This must be the Chrome Extension since otherwise we would have state descriptors defined.
                var key, descriptorKeys = Object.keys(powwow.descriptors);
                for (var i=0; i < descriptorKeys.length; i++) {
                    key = descriptorKeys[i];
                    if (powwow.descriptors[key].connect) {
                        powwow.descriptors[key].id = key;
                        models.push(powwow.descriptors[key]);
                    }
                }
            }
